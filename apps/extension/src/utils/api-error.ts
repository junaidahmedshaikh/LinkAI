...
export function extractApiErrorMessage(error: unknown, fallback = "Something went wrong"): string {
  ...
  if (error instanceof Error && error.message) {
    if (/DeepSeek API error \(401\)|invalid authentication credentials/i.test(error.message)) {
      return "AI service authentication failed on the server. Ask your admin to set a valid DEEPSEEK_API_KEY in the backend .env file.";
    }
